...
from ppreproc_pfb import PFBFormatError, PFBReader

from tools.generate_fixture import generate
...
class ReaderTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.pfb, cls.pfc = generate(Path(__file__).parent / "fixtures")
    ...
    def test_pfc_peak_count_mismatch_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with PFBReader(target_pfb) as reader, self.assertRaises(PFBFormatError):
    ...
    def test_cli_version(self) -> None:
        ...
        environment["PYTHONPATH"] = str(SRC)
        completed = subprocess.run(
            [sys.executable, "-m", "ppreproc_pfb", "--version"],
            check=True,
            capture_output=True,
            text=True,
            env=environment,
        )
        self.assertEqual(completed.stdout.strip(), "ppreproc-pfb 1.0.0")
